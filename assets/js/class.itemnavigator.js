...
class CItemNavigatorRME {
	...
	constructor(config) {
		...
		this.#container.classList.add(CItemNavigatorRME.ZBX_STYLE_CLASS);
	...
	setValue({items, hosts, is_limit_exceeded, selected_itemid}) {
		...
		this.#navigation_tree = new CNavigationTreeRME(this.#nodes, {
			selected_id: selected_itemid,
			show_problems: this.#config.show_problems,
			severities: this.#config.severities,
			show_groupings_only: this.#config.show_groupings_only
		});
	...
	#prepareNodesProperties(nodes) {
		for (let i = 0; i < nodes.length; i++) {
			if (nodes[i].children === undefined) {
				...
				nodes[i].is_open = this.#config.open_groups.includes(JSON.stringify(nodes[i].group_identifier.group_identifier));
	...
	#createGroup(item, level = 0, parent = null) {
		...
		switch (attribute.attribute) {
			case CItemNavigatorRME.GROUP_BY_HOST_GROUP:
				for (const hostgroup of this.#hosts[item.hostid].hostgroups) {
					const new_group = {
						...CItemNavigatorRME.#getGroupTemplate(),
						name: hostgroup.name,
						group_by: {
							name: t('Host group')
						},
						group_identifier: {
							group_identifier: parent !== null
								? [...parent.group_identifier.group_identifier, hostgroup.name]
								: [hostgroup.name],
							itemids: [item.itemid]
						},
						level
					...
					if (parent !== null) {
						if (!parent.group_identifier.itemids.includes(item.itemid)) {
							parent.group_identifier.itemids.push(item.itemid);
						}
					}
			...
			case CItemNavigatorRME.GROUP_BY_HOST_NAME:
				const new_group = {
					...CItemNavigatorRME.#getGroupTemplate(),
					...
					group_identifier: {
						group_identifier: parent !== null
							? [...parent.group_identifier.group_identifier, this.#hosts[item.hostid].name]
							: [this.#hosts[item.hostid].name],
						itemids: [item.itemid]
					},
					level
				};
			...
			case CItemNavigatorRME.GROUP_BY_HOST_TAG_VALUE:
			case CItemNavigatorRME.GROUP_BY_ITEM_TAG_VALUE:
				let tags = this.#hosts[item.hostid].tags || [];
				let attribute_name = t('Host tag');

				if (attribute.attribute === CItemNavigatorRME.GROUP_BY_ITEM_TAG_VALUE) {
					tags = item.tags;
				...
				if (matching_tags.length === 0) {
					const new_group = {
						...
						group_identifier: {
							group_identifier: parent !== null
								? [...parent.group_identifier.group_identifier, null]
								: [null],
							itemids: [item.itemid]
						},
						level,
					...
					for (const tag of matching_tags) {
						const new_group = {
							...
							group_identifier: {
								group_identifier: parent !== null
									? [...parent.group_identifier.group_identifier, tag.value]
									: [tag.value],
								itemids: [item.itemid]
							},
							level
	...
	#insertGroup(new_group, parent, level, item) {
		...
		if (level === this.#config.group_by.length - 1) {
			if (!new_group.children.some(child => child.itemid === item.itemid)) {
				if (!new_group.group_identifier.itemids.includes(item.itemid)) {
					new_group.group_identifier.itemids.push(item.itemid);
				}
	...
	#createLimit(limit) {
		...
		element.classList.add(CItemNavigatorRME.ZBX_STYLE_LIMIT);
	...
	#registerListeners() {
		this.#listeners = {
			itemSelect: e => {
				this.#container.dispatchEvent(new CustomEvent(CItemNavigatorRME.EVENT_ITEM_SELECT, {
					detail: {
						itemid: e.detail.id
					}
				}));
			...
			groupToggle: e => {
				const selected_group_identifier = e.detail.group_identifier.group_identifier;
				...
				this.#container.dispatchEvent(new CustomEvent(CItemNavigatorRME.EVENT_GROUP_TOGGLE, {
					detail: {
						group_identifier: e.detail.group_identifier.group_identifier,
	 ...
	 */
	#activateListeners() {
		this.#navigation_tree.getContainer().addEventListener(CNavigationTreeRME.EVENT_ITEM_SELECT,
			this.#listeners.itemSelect
		);
		this.#navigation_tree.getContainer().addEventListener(CNavigationTreeRME.EVENT_GROUP_TOGGLE,
