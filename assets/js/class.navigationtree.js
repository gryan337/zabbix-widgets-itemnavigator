...
class CNavigationTreeRME {
	...
	static ZBX_STYLE_NAV_HOVER =									'nav-hoverable-itemn';
	...
	/**
	 * Create CNavigationTreeRME instance.
	 *
	 ...
	 *
	 * @returns {CNavigationTreeRME}
	 */
	constructor(nodes, {
		selected_id = '',
		show_problems = true,
		severities = [],
		show_groupings_only = false
	} = {}) {
		this.#selected_id = selected_id;
		this.#show_problems = show_problems;
		this.#severities = severities;
		this.show_groupings_only = show_groupings_only;

		this.#container = document.createElement('div');
		this.#container.classList.add(CNavigationTreeRME.ZBX_STYLE_CLASS);

		this.#container_nodes = document.createElement('div');
		this.#container_nodes.classList.add(CNavigationTreeRME.ZBX_STYLE_NODES);
		...
		for (const node of nodes) {
			...
			if (el) {
				this.#container_nodes.appendChild(el);
			}
		}
		...
		if (this.#selected_id !== '') {
			const node = this.#container.querySelector(`
				.${CNavigationTreeRME.ZBX_STYLE_NODE}[data-id="${this.#selected_id}"]
			`);
	 ...
	 */
	#createNode(node) {
		// Skip leaf nodes if show_groupings_only is true
		if (this.show_groupings_only && (!node.children || node.children.length === 0)) {
			return null;
		}

		const container = document.createElement('div');
		container.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE);
		container.dataset.level = node.level;

		if (node.children?.length > 0) {
			container.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_IS_GROUP);
			container.dataset.group_identifier = JSON.stringify(node.group_identifier);

			if (node.is_open) {
				container.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_IS_OPEN);
			}

			if (node.is_uncategorized) {
				container.classList.add(CNavigationTreeRME.ZBX_STYLE_GROUP_UNCATEGORIZED);
			}
		}
		else {
			container.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_IS_ITEM);
			container.dataset.id = node.id;
		}

		const info = document.createElement('div');
		info.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO);
		info.classList.add(CNavigationTreeRME.ZBX_STYLE_NAV_HOVER);

		container.appendChild(info);

		const helpers = document.createElement('div');
		helpers.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_HELPERS);

		info.appendChild(helpers);

		const primary = document.createElement('div');
		primary.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_PRIMARY);

		info.appendChild(primary);

		const secondary = document.createElement('div');
		secondary.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_SECONDARY);
		...
		if (node.children?.length > 0) {
			...
			children.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_CHILDREN);

			container.appendChild(children);

			for (const child of node.children) {
				const childEl = this.#createNode(child);
				if (childEl !== null) {
	...
	#createLevel() {
		...
		level.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_LEVEL);
	...
	#createArrow(node) {
		...
		arrow.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_ARROW);
	...
	#createName(node) {
		...
		name.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_NAME);
	...
	#setGroupHint(element, node) {
		...
		hint.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_GROUP_HINT);

		const attribute = document.createElement('span');
		attribute.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_GROUP_HINT_ATTRIBUTE);
		attribute.innerText = `${node.group_by.name}: `;

		hint.appendChild(attribute);

		const value = document.createElement('span');
		value.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_GROUP_HINT_VALUE);
		value.innerText = node.name;

		if (node.is_uncategorized) {
			value.classList.add(CNavigationTreeRME.ZBX_STYLE_GROUP_UNCATEGORIZED);
		}
	...
	#createMaintenance(node) {
		...
		maintenance.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_MAINTENANCE);
	...
	#createProblems(node) {
		...
		problems.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_PROBLEMS);
	...
	#setProblemsHint(element, problems) {
		...
		const hint = document.createElement('div');
		hint.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_PROBLEMS_HINT);

		for (const severity of problems) {
			const row = document.createElement('div');
			row.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_PROBLEMS_HINT_SEVERITY);

			hint.appendChild(row);

			const color = document.createElement('span');
			color.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_PROBLEMS_HINT_SEVERITY_COLOR, severity.style);

			row.appendChild(color);

			const name = document.createElement('span');
			name.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_PROBLEMS_HINT_SEVERITY_NAME);
			name.innerText = `${severity.label}: `;

			row.appendChild(name);

			const count = document.createElement('span');
			count.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_INFO_PROBLEMS_HINT_SEVERITY_COUNT);
	...
	#registerListeners() {
		this.#listeners = {
			containerClick: e => {
				...
				const node = e.target.closest(`.${CNavigationTreeRME.ZBX_STYLE_NODE}`);

				if (node === null) {
					return;
				}

				if (node.classList.contains(CNavigationTreeRME.ZBX_STYLE_NODE_IS_ITEM)) {
					this.selectItem(node.dataset.id);
				}
				else if (e.target.closest(`.${CNavigationTreeRME.ZBX_STYLE_NODE_INFO_ARROW} button`)) {
	...
	selectItem(item_id) {
		...
		const item_nodes = this.#container.querySelectorAll(`.${CNavigationTreeRME.ZBX_STYLE_NODE_IS_ITEM}`);

		for (const item_node of item_nodes) {
			if (item_node.dataset.id === this.#selected_id) {
				item_node.classList.add(CNavigationTreeRME.ZBX_STYLE_NODE_IS_SELECTED);

				const item_group = item_node.closest(`.${CNavigationTreeRME.ZBX_STYLE_NODE_IS_GROUP}`);
				...
				item_node.classList.remove(CNavigationTreeRME.ZBX_STYLE_NODE_IS_SELECTED);
			}
		}

		this.#container.dispatchEvent(new CustomEvent(CNavigationTreeRME.EVENT_ITEM_SELECT, {
			detail: {
				id: this.#selected_id
			}
		}));
	...
	#toggleGroup(node, force = null) {
		const toggle_group = (element, is_open) => {
			element.classList.toggle(CNavigationTreeRME.ZBX_STYLE_NODE_IS_OPEN, is_open);

			const arrow = element.querySelector(`.${CNavigationTreeRME.ZBX_STYLE_NODE_INFO_ARROW} span`);
			arrow.classList.toggle(ZBX_STYLE_ARROW_DOWN, is_open);
			arrow.classList.toggle(ZBX_STYLE_ARROW_RIGHT, !is_open);

			if (!is_open) {
				for (const inner_open_node of node.querySelectorAll(`.${CNavigationTreeRME.ZBX_STYLE_NODE_IS_OPEN}`)) {
					toggle_group(inner_open_node, false);
				}
			}
		}

		const is_open = force ?? !node.classList.contains(CNavigationTreeRME.ZBX_STYLE_NODE_IS_OPEN);
		...
		if (is_open) {
			...
			while (parent !== null && !parent.classList.contains(CNavigationTreeRME.ZBX_STYLE_CLASS)) {
				if (parent.classList.contains(CNavigationTreeRME.ZBX_STYLE_NODE_IS_GROUP)) {
		...
		this.#container.dispatchEvent(new CustomEvent(CNavigationTreeRME.EVENT_GROUP_TOGGLE, {
			detail: {
